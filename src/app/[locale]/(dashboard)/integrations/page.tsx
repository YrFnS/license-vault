	...
	return (
		<div className="space-y-6 p-4 md:p-6 max-w-7xl mx-auto">
			...
			>
				<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
					...
					<Button
						...
						onClick={() => h.setConnectDialogOpen(true)}
					>
			...
			<CategoryTabs
				...
				syncingIds={h.syncingIds}
				onSync={h.handleSync}
				onDisconnect={(intg) => {
					h.setDisconnectTarget(intg);
					h.setDisconnectDialogOpen(true);
				}}
				onConnect={h.handleOpenConnect}
			/>

			<ConnectDialog
				open={h.connectDialogOpen}
				onOpenChange={h.setConnectDialogOpen}
				selectedIntegration={h.selectedIntegration}
				apiKey={h.apiKey}
				onApiKeyChange={h.setApiKey}
				baseUrl={h.baseUrl}
				onBaseUrlChange={h.setBaseUrl}
				syncFrequency={h.syncFrequency}
				onSyncFrequencyChange={h.setSyncFrequency}
				dataMappings={h.dataMappings}
				onDataMappingsChange={h.setDataMappings}
				testResult={h.testResult}
				testMessage={h.testMessage}
				testing={h.testing}
				connecting={h.connecting}
				onTestConnection={h.handleTestConnection}
				onConnect={h.handleConnect}
			/>

			<DisconnectDialog
				open={h.disconnectDialogOpen}
				onOpenChange={h.setDisconnectDialogOpen}
				target={h.disconnectTarget}
				onConfirm={h.handleDisconnect}
			/>
		</div>
	);
}
