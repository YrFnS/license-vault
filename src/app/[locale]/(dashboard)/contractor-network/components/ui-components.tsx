		...
		>
			<svg width={size} height={size} className="-rotate-90">
				<circle cx={size / 2} cy={size / 2} r={r} fill="none"
					stroke="currentColor" strokeWidth={4} className="text-muted/30" />
				<circle cx={size / 2} cy={size / 2} r={r} fill="none"
...
export function TradeBadge({ tradeType }: { tradeType: string }) {
	...
	const label = t(tradeType as any) || tradeType;
	return (
...
/* ── TaggableBadges helper used in Detail dialog ───────────── */
export function TaggableBadges({ json, label, t, badgeClassName }: {
	json: string | null;
	label: string;
	t: (key: string) => string;
	...
	if (!json) return null;
	let items: string[] = [];
	try { items = JSON.parse(json); } catch { /* ignore */ }
	...
	return (
		<div className="mt-3">
			...
			<div className="flex flex-wrap gap-1">
				{items.map((s, i) => (
					<Badge key={i} variant="outline" className={cn("text-xs", badgeClassName)}>{s}</Badge>
				))}
			</div>
		</div>
	);
