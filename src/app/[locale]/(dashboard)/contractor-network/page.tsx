...
const TRADE_TYPES = ['electrical', 'plumbing', 'hvac', 'general', 'roofing', 'concrete', 'painting', 'landscaping', 'other'];
...
export default function ContractorNetworkPage() {
  ...
  const resetForm = useCallback(() => {
    setForm({
      companyName: '', tradeType: 'general', licenseNumber: '', licenseState: '', licenseStatus: 'unknown',
      licenseExpiry: '', contactName: '', contactEmail: '', contactPhone: '', address: '', city: '',
      state: '', zip: '', website: '', insuranceProvider: '', insuranceExpiry: '', insuranceStatus: 'unknown',
      bondingCapacity: '', totalProjects: '', completedProjects: '', rating: '', yearsInBusiness: '',
      employeeCount: '', notes: '',
    });
  }, []);
  ...
  const handleExport = useCallback(() => {
    const headers = ['Company Name', 'Trade Type', 'License Number', 'License State', 'License Status', 'Contact Name', 'Contact Email', 'Contact Phone', 'City', 'State', 'Compliance Score', 'Rating', 'Verified', 'Preferred'];
    const rows = contractors.map(c => [
      c.companyName, c.tradeType, c.licenseNumber || '', c.licenseState || '', c.licenseStatus,
      c.contactName || '', c.contactEmail || '', c.contactPhone || '', c.city || '', c.state || '',
      c.complianceScore, c.rating, c.isVerified ? 'Yes' : 'No', c.isPreferred ? 'Yes' : 'No',
    ]);
      ...
      <div className="flex items-center justify-center min-h-[50vh]">
        <Card className="max-w-md w-full">
          <CardContent className="p-6 text-center">
            ...
            <Button onClick={() => { setError(null); fetchContractors(); }} variant="outline" className="mt-4">{tc('retry')}</Button>
          </CardContent>
        </Card>
      </div>
    );
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      <motion.div {...fadeIn} className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        {canManage && (
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" onClick={() => { resetForm(); setAddOpen(true); }} className="bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white shadow-sm shadow-emerald-500/25">
      ...
      {/* Stats Cards */}
      <motion.div variants={staggerContainer} initial="initial" animate="animate" className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
        {statsCards.map((stat) => {
      ...
      <motion.div {...fadeIn} className="space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          <div className="relative flex-1 sm:max-w-md">
            ...
            <Input placeholder={t('search')} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="ps-9 h-9 bg-muted/30 border-border/50" />
          </div>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => setShowFilters(!showFilters)} className={cn(showFilters && 'bg-emerald-50 border-emerald-300 text-emerald-700 dark:bg-emerald-950/30 dark:border-emerald-700 dark:text-emerald-400')}>
              <Zap className="size-4 me-1" />Filters
              <ChevronDown className={cn('size-4 ms-1 transition-transform', showFilters && 'rotate-180')} />
            </Button>
            <div className="flex items-center border rounded-lg overflow-hidden">
              <Button variant={viewMode === 'grid' ? 'default' : 'ghost'} size="icon" className={cn('size-9 rounded-none', viewMode === 'grid' && 'bg-gradient-to-r from-emerald-500 to-teal-600 text-white')} onClick={() => setViewMode('grid')}>
                <LayoutGrid className="size-4" /><span className="sr-only">{t('gridView')}</span>
              </Button>
              <Button variant={viewMode === 'table' ? 'default' : 'ghost'} size="icon" className={cn('size-9 rounded-none', viewMode === 'table' && 'bg-gradient-to-r from-emerald-500 to-teal-600 text-white')} onClick={() => setViewMode('table')}>
                <List className="size-4" /><span className="sr-only">{t('tableView')}</span>
              </Button>
            </div>
          </div>
        </div>
                ...
                <CardContent className="p-4">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div>
                      ...
                      <Select value={tradeFilter} onValueChange={setTradeFilter}>
                        ...
                        <SelectContent>
                          ...
                          {TRADE_TYPES.map(tt => <SelectItem key={tt} value={tt}>{t(tt as any) || tt}</SelectItem>)}
                        </SelectContent>
                      </Select>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <Select value={insuranceStatusFilter} onValueChange={setInsuranceStatusFilter}>
                        <SelectTrigger className="h-9 text-sm"><SelectValue /></SelectTrigger>
                        <SelectContent>
                          <SelectItem value="all">All</SelectItem>
      ...
      {/* Bulk Action Bar */}
      {selectedIds.size > 0 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="fixed bottom-6 start-1/2 -translate-x-1/2 z-50">
          <div className="flex items-center gap-3 px-5 py-3 rounded-xl bg-card border border-border shadow-lg">
            <span className="text-sm font-medium">{selectedIds.size} selected</span>
            {canManage && (
              <>
                <Button size="sm" onClick={handleBulkVerify} disabled={saving} className="bg-gradient-to-r from-emerald-500 to-teal-600 text-white">
      ...
      {contractors.length === 0 ? (
        <motion.div {...fadeIn} className="flex flex-col items-center justify-center py-20 text-center">
          ...
          {canManage && (
            <Button size="sm" className="mt-4 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white shadow-sm shadow-emerald-500/20" onClick={() => { resetForm(); setAddOpen(true); }}>
        ...
        <>
          {/* Grid View */}
          <motion.div variants={staggerContainer} initial="initial" animate="animate" className={cn(viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4' : 'hidden')}>
            {contractors.map((c) => (
              <motion.div key={c.id} variants={staggerItem} whileHover={{ y: -2 }} transition={{ type: 'spring', stiffness: 400, damping: 25 }}>
                <Card className={cn('shadow-sm hover:shadow-md transition-all duration-300 group cursor-pointer', selectedIds.has(c.id) && 'ring-2 ring-emerald-500/50', c.isBlacklisted && 'opacity-60')} onClick={() => openDetail(c)}>
                  <CardContent className="p-4">
                    <div className="flex items-start gap-3">
                      <div className="shrink-0 mt-0.5">
                        {canManage && <Checkbox checked={selectedIds.has(c.id)} onCheckedChange={() => toggleSelect(c.id)} onClick={(e) => e.stopPropagation()} className="mb-2" />}
                    ...
                    <div className="flex items-center justify-end gap-1 mt-3 pt-2 border-t">
                      <Button variant="ghost" size="sm" className="text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-950/30 h-7 text-xs" onClick={(e) => { e.stopPropagation(); openDetail(c); }}>
                        <Eye className="size-3 me-1" />{t('viewDetails')}
                      </Button>
                      {canManage && !c.isVerified && (
                        <Button variant="ghost" size="sm" className="text-teal-600 hover:bg-teal-50 dark:hover:bg-teal-950/30 h-7 text-xs" onClick={(e) => { e.stopPropagation(); setSelectedContractor(c); setVerifyDialogOpen(true); }}>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b bg-muted/50">
                        <th className="p-3 w-10"><Checkbox checked={selectedIds.size === contractors.length && contractors.length > 0} onCheckedChange={toggleSelectAll} /></th>
                    ...
                    <tbody>
                      <AnimatePresence>
                        {contractors.map((c) => (
                          <motion.tr key={c.id} variants={staggerItem} initial="initial" animate="animate" className={cn('border-b last:border-0 transition-colors duration-150 cursor-pointer', selectedIds.has(c.id) ? 'bg-emerald-50/50 dark:bg-emerald-950/20' : 'hover:bg-muted/30', c.isBlacklisted && 'opacity-60')} onClick={() => openDetail(c)}>
                            <td className="p-3"><Checkbox checked={selectedIds.has(c.id)} onCheckedChange={() => toggleSelect(c.id)} onClick={(e) => e.stopPropagation()} /></td>
                            <td className="p-3">
                              <div className="flex items-center gap-2">
                                <div>
                            ...
                            <td className="p-3">
                              <div className="flex items-center justify-end gap-1" onClick={(e) => e.stopPropagation()}>
                                <Button variant="ghost" size="icon" className="size-8 hover:bg-emerald-50 hover:text-emerald-600 dark:hover:bg-emerald-950/30" onClick={() => openDetail(c)}>
                                  <Eye className="size-4" /><span className="sr-only">{tc('viewDetails')}</span>
                                </Button>
                                {canManage && !c.isVerified && (
                                  <Button variant="ghost" size="icon" className="size-8 hover:bg-teal-50 hover:text-teal-600 dark:hover:bg-teal-950/30" onClick={() => { setSelectedContractor(c); setVerifyDialogOpen(true); }}>
                                    <ShieldCheck className="size-4" /><span className="sr-only">{t('verify')}</span>
                                  </Button>
                                )}
                                {canManage && (
                                  <Button variant="ghost" size="icon" className={cn('size-8', c.isBlacklisted ? 'hover:bg-emerald-50 hover:text-emerald-600' : 'hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-950/30')} onClick={() => { setSelectedContractor(c); setBlacklistDialogOpen(true); }}>
                                    <Ban className="size-4" /><span className="sr-only">{t('blacklistedBadge')}</span>
                                  </Button>
                                )}
                              </div>
                            </td>
        ...
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
            <div className="space-y-2">
              ...
              <Input value={form.companyName} onChange={(e) => setForm(f => ({ ...f, companyName: e.target.value }))} placeholder="Acme Electrical" />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2"><Label>License Number</Label><Input value={form.licenseNumber} onChange={(e) => setForm(f => ({ ...f, licenseNumber: e.target.value }))} /></div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Select value={form.licenseStatus} onValueChange={(v) => setForm(f => ({ ...f, licenseStatus: v }))}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="active">Active</SelectItem>
                  <SelectItem value="expired">Expired</SelectItem>
            ...
            <div className="space-y-2"><Label>License Expiry</Label><Input type="date" value={form.licenseExpiry} onChange={(e) => setForm(f => ({ ...f, licenseExpiry: e.target.value }))} /></div>
            <div className="space-y-2"><Label>Contact Name</Label><Input value={form.contactName} onChange={(e) => setForm(f => ({ ...f, contactName: e.target.value }))} /></div>
            ...
            <div className="space-y-2"><Label>ZIP</Label><Input value={form.zip} onChange={(e) => setForm(f => ({ ...f, zip: e.target.value }))} /></div>
            <div className="space-y-2"><Label>Website</Label><Input value={form.website} onChange={(e) => setForm(f => ({ ...f, website: e.target.value }))} /></div>
            <div className="space-y-2"><Label>Insurance Provider</Label><Input value={form.insuranceProvider} onChange={(e) => setForm(f => ({ ...f, insuranceProvider: e.target.value }))} /></div>
            ...
            <div className="space-y-2"><Label>Bonding Capacity ($)</Label><Input type="number" value={form.bondingCapacity} onChange={(e) => setForm(f => ({ ...f, bondingCapacity: e.target.value }))} /></div>
            <div className="space-y-2"><Label>Total Projects</Label><Input type="number" value={form.totalProjects} onChange={(e) => setForm(f => ({ ...f, totalProjects: e.target.value }))} /></div>
            <div className="space-y-2"><Label>Completed Projects</Label><Input type="number" value={form.completedProjects} onChange={(e) => setForm(f => ({ ...f, completedProjects: e.target.value }))} /></div>
            <div className="space-y-2"><Label>Rating (0-5)</Label><Input type="number" min="0" max="5" step="0.1" value={form.rating} onChange={(e) => setForm(f => ({ ...f, rating: e.target.value }))} /></div>
            <div className="space-y-2"><Label>Years in Business</Label><Input type="number" value={form.yearsInBusiness} onChange={(e) => setForm(f => ({ ...f, yearsInBusiness: e.target.value }))} /></div>
            <div className="space-y-2">
            ...
            <div className="space-y-2 md:col-span-2"><Label>Notes</Label><Textarea value={form.notes} onChange={(e) => setForm(f => ({ ...f, notes: e.target.value }))} rows={3} /></div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setAddOpen(false)}>{tc('cancel')}</Button>
            <Button onClick={handleSave} disabled={saving} className="bg-gradient-to-r from-emerald-500 to-teal-600 text-white">
              {saving ? <Loader2 className="size-4 animate-spin me-1" /> : <Plus className="size-4 me-1" />}{tc('create')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          {detailContractor && (
            <>
              <DialogHeader>
                <div className="flex items-center gap-2">
                  <DialogTitle className="text-xl">{detailContractor.companyName}</DialogTitle>
                  {detailContractor.isVerified && <Badge className="bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400 border-emerald-200"><Verified className="size-3 me-1" />{t('verifiedBadge')}</Badge>}
                  {detailContractor.isPreferred && <Badge className="bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:text-amber-400 border-amber-200"><Award className="size-3 me-1" />{t('preferredBadge')}</Badge>}
                  {detailContractor.isBlacklisted && <Badge className="bg-red-100 text-red-700 dark:bg-red-950/40 dark:text-red-400 border-red-200"><Ban className="size-3 me-1" />{t('blacklistedBadge')}</Badge>}
                </div>
                <DialogDescription>
                  <div className="flex items-center gap-2 mt-1">
                    <TradeBadge tradeType={detailContractor.tradeType} />
                    {detailContractor.city && detailContractor.state && <span className="text-xs flex items-center gap-0.5"><MapPin className="size-3" />{detailContractor.city}, {detailContractor.state}</span>}
                  </div>
                </DialogDescription>
              </DialogHeader>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Card className="shadow-sm">
                    <CardContent className="p-4 flex items-center gap-4">
                      <ScoreRing score={detailContractor.complianceScore} size={64} />
                      <div>
                        <p className="font-semibold">{t('complianceScore')}</p>
                        <p className="text-2xl font-bold">{detailContractor.complianceScore}/100</p>
                      </div>
                    </CardContent>
                  </Card>
                  <Card className="shadow-sm">
                    <CardContent className="p-4">
                      ...
                      <div className="flex items-center gap-2">
                        <StarRating rating={detailContractor.rating} size={20} />
                        <span className="text-lg font-bold">{detailContractor.rating.toFixed(1)}</span>
                        <span className="text-sm text-muted-foreground">({detailContractor.reviewCount})</span>
                      </div>
                    </CardContent>
                  </Card>
                </div>
                        ...
                        {[
                          ...
                          <div key={item.label} className="flex items-center gap-3">
                            ...
                            <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                              <div className={cn('h-full rounded-full transition-all duration-500', (item.value / item.max) > 0.7 ? 'bg-emerald-500' : (item.value / item.max) > 0.4 ? 'bg-amber-500' : 'bg-red-500')} style={{ width: `${(item.value / item.max) * 100}%` }} />
                            </div>
                ...
                <Card className="shadow-sm">
                  <CardContent className="p-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                      {detailContractor.contactName && <div className="flex items-center gap-2"><Users className="size-4 text-muted-foreground" />{detailContractor.contactName}</div>}
                      {detailContractor.contactEmail && <div className="flex items-center gap-2"><Mail className="size-4 text-muted-foreground" />{detailContractor.contactEmail}</div>}
                      {detailContractor.contactPhone && <div className="flex items-center gap-2"><Phone className="size-4 text-muted-foreground" />{detailContractor.contactPhone}</div>}
                      {detailContractor.website && <div className="flex items-center gap-2"><Globe className="size-4 text-muted-foreground" /><a href={detailContractor.website} target="_blank" rel="noopener noreferrer" className="text-emerald-600 hover:underline">{detailContractor.website}</a></div>}
                      {detailContractor.address && <div className="flex items-center gap-2"><MapPin className="size-4 text-muted-foreground" />{detailContractor.address}{detailContractor.city ? `, ${detailContractor.city}` : ''}{detailContractor.state ? `, ${detailContractor.state}` : ''}{detailContractor.zip ? ` ${detailContractor.zip}` : ''}</div>}
                    </div>
                  </CardContent>
                </Card>
                  ...
                  <Card className="shadow-sm">
                    <CardContent className="p-4">
                      ...
                      <div className="space-y-1.5 text-sm">
                        <div className="flex justify-between"><span className="text-muted-foreground">Number</span><span className="font-medium">{detailContractor.licenseNumber || '—'}</span></div>
                        <div className="flex justify-between"><span className="text-muted-foreground">State</span><span className="font-medium">{detailContractor.licenseState || '—'}</span></div>
                        <div className="flex justify-between"><span className="text-muted-foreground">Status</span><LicenseStatusBadge status={detailContractor.licenseStatus} /></div>
                        <div className="flex justify-between"><span className="text-muted-foreground">Expiry</span><span className="font-medium">{detailContractor.licenseExpiry ? new Date(detailContractor.licenseExpiry).toLocaleDateString() : '—'}</span></div>
                      </div>
                    </CardContent>
                  </Card>
                  <Card className="shadow-sm">
                    <CardContent className="p-4">
                      ...
                      <div className="space-y-1.5 text-sm">
                        <div className="flex justify-between"><span className="text-muted-foreground">Provider</span><span className="font-medium">{detailContractor.insuranceProvider || '—'}</span></div>
                        <div className="flex justify-between"><span className="text-muted-foreground">Status</span><InsuranceStatusBadge status={detailContractor.insuranceStatus} /></div>
                        <div className="flex justify-between"><span className="text-muted-foreground">Expiry</span><span className="font-medium">{detailContractor.insuranceExpiry ? new Date(detailContractor.insuranceExpiry).toLocaleDateString() : '—'}</span></div>
                      </div>
                    </CardContent>
                  </Card>
                </div>
                ...
                <Card className="shadow-sm">
                  <CardContent className="p-4">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                      <div><span className="text-muted-foreground block text-xs">{t('bondingCapacity')}</span><span className="font-medium">${(detailContractor.bondingCapacity || 0).toLocaleString()}</span></div>
                      <div><span className="text-muted-foreground block text-xs">{t('yearsInBusiness')}</span><span className="font-medium">{detailContractor.yearsInBusiness}</span></div>
                      <div><span className="text-muted-foreground block text-xs">{t('employeeCount')}</span><span className="font-medium">{detailContractor.employeeCount || '—'}</span></div>
                      <div><span className="text-muted-foreground block text-xs">{t('completedProjects')}</span><span className="font-medium">{detailContractor.completedProjects}/{detailContractor.totalProjects}</span></div>
                      <div><span className="text-muted-foreground block text-xs">Verified</span><span className="font-medium">{detailContractor.isVerified ? 'Yes' : 'No'}{detailContractor.lastVerifiedAt ? ` (${new Date(detailContractor.lastVerifiedAt).toLocaleDateString()})` : ''}</span></div>
                    </div>
                    {detailContractor.specialties && (() => {
                      try {
                        const items = JSON.parse(detailContractor.specialties);
                        if (Array.isArray(items) && items.length > 0) {
                          return <div className="mt-3"><span className="text-xs text-muted-foreground block mb-1">{t('specialties')}</span><div className="flex flex-wrap gap-1">{items.map((s: string, i: number) => <Badge key={i} variant="outline" className="text-xs">{s}</Badge>)}</div></div>;
                        }
                      } catch {} return null;
                    })()}
                    {detailContractor.certifications && (() => {
                      try {
                        const items = JSON.parse(detailContractor.certifications);
                        if (Array.isArray(items) && items.length > 0) {
                          return <div className="mt-3"><span className="text-xs text-muted-foreground block mb-1">{t('certifications')}</span><div className="flex flex-wrap gap-1">{items.map((s: string, i: number) => <Badge key={i} variant="outline" className="text-xs bg-teal-50 text-teal-700 dark:bg-teal-950/30 dark:text-teal-400">{s}</Badge>)}</div></div>;
                        }
                      } catch {} return null;
                    })()}
                    {detailContractor.serviceAreas && (() => {
                      try {
                        const items = JSON.parse(detailContractor.serviceAreas);
                        if (Array.isArray(items) && items.length > 0) {
                          return <div className="mt-3"><span className="text-xs text-muted-foreground block mb-1">{t('serviceAreas')}</span><div className="flex flex-wrap gap-1">{items.map((s: string, i: number) => <Badge key={i} variant="outline" className="text-xs">{s}</Badge>)}</div></div>;
                        }
                      } catch {} return null;
                    })()}
                    {detailContractor.notes && <div className="mt-3"><span className="text-xs text-muted-foreground block mb-1">Notes</span><p className="text-sm bg-muted/50 rounded-md p-2">{detailContractor.notes}</p></div>}
                  </CardContent>
                </Card>
                ...
                {canManage && (
                  <div className="flex items-center gap-2 justify-end">
                    {!detailContractor.isVerified && (
                      <Button onClick={() => { setSelectedContractor(detailContractor); setVerifyDialogOpen(true); }} className="bg-gradient-to-r from-emerald-500 to-teal-600 text-white">
                        <ShieldCheck className="size-4 me-1" />{t('verify')}
                      </Button>
                    )}
                    <Button variant={detailContractor.isBlacklisted ? 'default' : 'destructive'} onClick={() => { setSelectedContractor(detailContractor); setBlacklistDialogOpen(true); }}>
                      <Ban className="size-4 me-1" />{detailContractor.isBlacklisted ? 'Remove Blacklist' : t('blacklistedBadge')}
                    </Button>
                  </div>
                )}
              </div>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="py-4 text-center">
            <div className="rounded-xl border-2 border-dashed border-border/50 p-8 hover:border-emerald-300 transition-colors">
              ...
              <p className="text-xs text-muted-foreground mt-1">Optional: licenseNumber, licenseState, licenseStatus, contactName, contactEmail, contactPhone, city, state, insuranceProvider, insuranceStatus, bondingCapacity, rating, yearsInBusiness</p>
            </div>
            <Button onClick={handleImport} disabled={importing} className="mt-4 bg-gradient-to-r from-emerald-500 to-teal-600 text-white">
              {importing ? <Loader2 className="size-4 animate-spin me-1" /> : <Upload className="size-4 me-1" />}Select CSV File
            </Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <AlertDialog open={verifyDialogOpen} onOpenChange={setVerifyDialogOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleVerify} className="bg-emerald-600 hover:bg-emerald-700">{t('verify')}</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog open={blacklistDialogOpen} onOpenChange={setBlacklistDialogOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel>{tc('cancel')}</AlertDialogCancel>
            <AlertDialogAction onClick={handleBlacklist} className="bg-red-600 hover:bg-red-700">{selectedContractor?.isBlacklisted ? 'Remove Blacklist' : t('blacklistedBadge')}</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
