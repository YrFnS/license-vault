	...
	const fetchApplications = useCallback(async () => {
		try {
			...
			if (typeFilter && typeFilter !== "all")
				params.set("applicationType", typeFilter);
	...
	const statCards = [
		{
			...
			value: counts.total,
			color:
				"from-teal-50 to-teal-100/40 dark:from-teal-950/40 dark:to-teal-900/20",
		...
		},
		{
			...
			value: counts.draft,
			color:
				"from-slate-50 to-slate-100/40 dark:from-slate-800/40 dark:to-slate-700/20",
		...
		},
		{
			...
			color:
				"from-blue-50 to-blue-100/40 dark:from-blue-950/40 dark:to-blue-900/20",
		...
		},
		{
			...
			color:
				"from-emerald-50 to-emerald-100/40 dark:from-emerald-950/40 dark:to-emerald-900/20",
		...
		},
		{
			...
			value: counts.denied,
			color:
				"from-red-50 to-red-100/40 dark:from-red-950/40 dark:to-red-900/20",
			...
			<div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
				{statCards.map((card) => {
					...
					return (
						...
						>
							<Card
								className={`bg-gradient-to-br ${card.color} border-s-4 ${card.accent} hover:shadow-md transition-shadow cursor-pointer`}
								onClick={() =>
							...
							>
								<CardContent className="p-3 md:p-4">
									...
									<p className="text-2xl lg:text-3xl font-extrabold tabular-nums">
										{card.value}
									</p>
								</CardContent>
							</Card>
				...
				<ScrollArea className="max-h-[600px]">
					<div className="space-y-3">
						{filteredApps.map((app, idx) => {
							...
							let checklistProgress: { docCount: number } | null = null;
							try {
							...
							return (
								...
								>
									<Card
										...
										onClick={() =>
											router.push(`/license-applications/${app.id}`)
										}
									>
										<CardContent className="p-4">
											<div className="flex items-start gap-4">
												<div
													className={`flex items-center justify-center size-10 rounded-xl ${config.bg} shrink-0`}
												>
