...
import { ShieldCheck, ChevronDown, CheckCircle2, XCircle, AlertCircle } from 'lucide-react';
...
export function AssessmentDialog({ open, onOpenChange, result }: AssessmentDialogProps) {
            ...
            <div className="space-y-6 pr-4">
              ...
              <div className="flex flex-col items-center gap-2 py-4">
                ...
                <div className="text-center">
                  <p className="text-sm font-semibold">{result.vendor.vendorName}</p>
                  <p className="text-xs text-muted-foreground">{getScoreLabel(result.vendor.overallScore)}</p>
                  {(() => {
                    const rc = RISK_BADGE_CONFIG[result.vendor.riskLevel] || RISK_BADGE_CONFIG.medium;
                    return <Badge className={`${rc.bg} ${rc.text} border-0 font-semibold`}>{t(result.vendor.riskLevel as 'critical' | 'high' | 'medium' | 'low')}</Badge>;
                  })()}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-1">
                  {catScores(result.vendor).map(cat => (
                    <ScoreBar key={cat.key} label={cat.label} score={result.vendor[cat.key as keyof VendorScoreData] as number} icon={cat.icon} weight={cat.weight} />
                  ))}
                </div>
              </div>
              ...
              {result.history.length > 1 && (
                <div>
                  ...
                  <ResponsiveContainer width="100%" height={120}>
                    <LineChart data={result.history.map(h => ({ date: new Date(h.date).toLocaleDateString(), score: h.score }))}>
