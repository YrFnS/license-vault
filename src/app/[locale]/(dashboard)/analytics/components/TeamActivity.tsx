				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
						...
						<div className="h-56">
							<ResponsiveContainer width="100%" height="100%">
								<BarChart data={teamData.actionsByUser.slice(0, 8)} margin={{ top: 5, right: 10, bottom: 5, left: 0 }}>
									...
									<XAxis dataKey="name" tick={{ fontSize: 10, fill: "hsl(var(--muted-foreground))" }} tickLine={false} axisLine={false} interval={0} angle={-30} textAnchor="end" height={50} />
					...
					<div>
						...
						<div className="h-56">
							<ResponsiveContainer width="100%" height="100%">
								<PieChart>
									<Pie data={teamData.actionTypes.slice(0, 6)} cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={3} dataKey="count" nameKey="action">
					...
					<div className="lg:col-span-2">
						...
						<div className="flex flex-wrap gap-3">
							{teamData.mostActiveUsers.map((user, idx) => (
								<div key={user.id} className="flex items-center gap-2.5 px-3 py-2 rounded-lg bg-muted/30 border border-border/50">
									<Avatar className="size-8 ring-1 ring-emerald-500/20">
										<AvatarFallback className="text-xs bg-gradient-to-br from-emerald-500/20 to-teal-500/20 text-emerald-700 dark:text-emerald-400 font-bold">{user.name.charAt(0).toUpperCase()}</AvatarFallback>
									</Avatar>
									<div>
										<p className="text-sm font-medium">{user.name}</p>
										<p className="text-xs text-muted-foreground">{user.count} {t("team.actions").toLowerCase()}</p>
									</div>
									{idx === 0 && <Badge className="bg-emerald-100 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800 text-[10px]">#1</Badge>}
								</div>
							))}
						</div>
					</div>
