...
export async function PUT(
  ...
  try {
    ...
    if (!['owner', 'admin'].includes(orgMember.role)) {
      return NextResponse.json(
        { error: 'Insufficient permissions. Only owners and admins can approve or reject requests.' },
        { status: 403 }
      );
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (approval.requestedBy !== userId && !['owner', 'admin'].includes(orgMember.role)) {
      return NextResponse.json(
        { error: 'Insufficient permissions. Only the requester or admins can cancel.' },
        { status: 403 }
      );
    }
