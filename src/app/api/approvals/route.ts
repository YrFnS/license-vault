...
export async function GET(request: Request) {
	try {
		...
		const requesterIds = [
			...new Set(
				approvals.map((a) => a.requestedBy).filter((id): id is string => !!id),
			),
		];
		...
		const reviewerIds = [
			...new Set(
				approvals.map((a) => a.reviewedBy).filter((id): id is string => !!id),
			),
		];
		...
		const approvalsEnriched = approvals.map((a) => ({
			...
			requesterName: a.requestedBy
				? requesterMap.get(a.requestedBy)?.name || null
				: null,
			requesterEmail: a.requestedBy
				? requesterMap.get(a.requestedBy)?.email || null
				: null,
			reviewerName: a.reviewedBy
				? reviewerMap.get(a.reviewedBy)?.name || null
				: null,
			reviewerEmail: a.reviewedBy
				? reviewerMap.get(a.reviewedBy)?.email || null
				: null,
		}));
