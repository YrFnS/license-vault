...
export async function GET(request: Request) {
	try {
		...
		if (
			complianceFilter &&
			["compliant", "deficient", "expired", "pending"].includes(
				complianceFilter,
			)
		) {
		...
		const activeCount = allRecords.filter(
			(r) => computeInsuranceStatus(r.expirationDate) === "active",
		).length;
		const expiringCount = allRecords.filter(
			(r) => computeInsuranceStatus(r.expirationDate) === "expiring_soon",
		).length;
		const expiredCount = allRecords.filter(
			(r) => computeInsuranceStatus(r.expirationDate) === "expired",
		).length;
		const totalCoverage = allRecords.reduce(
			(sum, r) => sum + r.coverageAmount,
			0,
		);
		const totalPremium = allRecords.reduce(
			(sum, r) => sum + r.premiumAmount,
			0,
		);

		// Compute compliance summary
		const compliantCount = allRecords.filter(
			(r) => computeComplianceStatus(r) === "compliant",
		).length;
		const deficientCount = allRecords.filter(
			(r) => computeComplianceStatus(r) === "deficient",
		).length;
		const expiredComplianceCount = allRecords.filter(
			(r) => computeComplianceStatus(r) === "expired",
		).length;
		const pendingCount = allRecords.filter(
			(r) => computeComplianceStatus(r) === "pending",
		).length;
	...
	try {
		...
		if (!["owner", "admin"].includes(orgMember.role as string)) {
			return NextResponse.json(
				{
					error:
						"Insufficient permissions. Only owners and admins can create records.",
				},
				{ status: 403 },
			);
		}
		...
		const sanitizedHolderName = holderName
			? sanitizeString(holderName)
			: undefined;
