  ...
  try {
    ...
    for (const ib of insuranceBonds) {
      ...
      for (const project of projects) {
        if (project.insuranceRequired > 0 && ib.type === 'insurance') {
          if (ib.coverageAmount < project.insuranceRequired) {
            deficiencies.push({
              insuranceId: ib.id,
              insuranceName: ib.name,
              type: ib.type,
              ...
              details: `Coverage $${ib.coverageAmount.toLocaleString()} is below project requirement $${project.insuranceRequired.toLocaleString()} for "${project.name}"`,
              severity: 'critical',
            });
          }
        }
    ...
    for (const project of projects) {
      if (project.insuranceRequired > 0) {
        const hasActiveInsurance = insuranceBonds.some(
          (ib) => ib.type === 'insurance' && computeInsuranceStatus(ib.expirationDate) === 'active'
        );
      ...
      if (project.bondRequired && project.bondAmount > 0) {
        const hasActiveBond = insuranceBonds.some(
          (ib) => ib.type === 'bond' && computeInsuranceStatus(ib.expirationDate) === 'active' && ib.coverageAmount >= project.bondAmount
        );
        if (!hasActiveBond) {
          deficiencies.push({
            insuranceId: '',
            ...
            details: `Project "${project.name}" requires a bond of $${project.bondAmount.toLocaleString()} but no active qualifying bond exists`,
            severity: 'critical',
          });
        }
      }
