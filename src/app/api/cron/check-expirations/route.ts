...
import { runFullCheck, createAutomationRun, completeAutomationRun, updateLastRunAt, getOrCreateAutomationSettings } from '@/lib/automation';
 ...
 */
function thresholdToPrefField(threshold: ExpirationThreshold): 'alert60Days' | 'alert30Days' | 'alert5Days' {
  switch (threshold) {
    case 60: return 'alert60Days';
    case 30: return 'alert30Days';
    case 5: return 'alert5Days';
  }
}
 ...
 */
function buildDedupKey(prefix: string, threshold: ExpirationThreshold, entityId: string): string {
 ...
 */
async function isAlertAlreadySent(userId: string, dedupKey: string): Promise<boolean> {
  ...
  try {
    ...
    for (const org of orgs) {
      ...
      try {
        ...
        summary.licensesChecked += results.licensesExpiring + results.licensesExpired;
        summary.insuranceChecked += results.insuranceExpiring + results.insuranceExpired;
    ...
    for (const license of expiringLicenses) {
      ...
      for (const member of license.org.members) {
        ...
        const dedupKey = buildDedupKey('EXPIRATION_ALERT_', threshold, license.id);
        ...
        if (alertPref.alertEmail) {
          try {
            ...
            const msg = err instanceof Error ? err.message : 'Failed to send license expiration email';
            summary.errors.push(`License ${license.id} → ${user.email}: ${msg}`);
          }
        }
      }
    }
    ...
    for (const record of expiringInsurance) {
      ...
      for (const member of record.org.members) {
        ...
        const dedupKey = buildDedupKey('INSURANCE_ALERT_', threshold, record.id);
        ...
        if (alertPref.alertEmail) {
          try {
            const expirationDateStr = record.expirationDate.toLocaleDateString();
            ...
            const msg = err instanceof Error ? err.message : 'Failed to send insurance expiration email';
            summary.errors.push(`Insurance ${record.id} → ${user.email}: ${msg}`);
          }
        }
      }
    }
...
export async function GET(request: Request) {
  try {
    ...
    const [
      ...
      db.license.count({ where: { expirationDate: { lt: now }, isRenewed: false } }),
      db.license.count({ where: { expirationDate: { gte: now, lte: fiveDaysFromNow }, isRenewed: false } }),
      db.license.count({ where: { expirationDate: { gt: fiveDaysFromNow, lte: thirtyDaysFromNow }, isRenewed: false } }),
      db.license.count({ where: { expirationDate: { gt: thirtyDaysFromNow, lte: sixtyDaysFromNow }, isRenewed: false } }),
      db.insuranceBond.count({ where: { expirationDate: { lt: now }, status: { not: 'expired' } } }),
      db.insuranceBond.count({ where: { expirationDate: { gte: now, lte: fiveDaysFromNow }, status: { not: 'expired' } } }),
      db.insuranceBond.count({ where: { expirationDate: { gt: fiveDaysFromNow, lte: thirtyDaysFromNow }, status: { not: 'expired' } } }),
      db.insuranceBond.count({ where: { expirationDate: { gt: thirtyDaysFromNow, lte: sixtyDaysFromNow }, status: { not: 'expired' } } }),
    ]);

    // Also get automation status for each org
    const automationSettings = await db.automationSetting.findMany();
    const lastRun = automationSettings.length > 0
      ? automationSettings.reduce((latest, s) =>
          s.lastRunAt && (!latest || s.lastRunAt > latest) ? s.lastRunAt : latest,
          null as Date | null
        )
      : null;
