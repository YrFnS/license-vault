...
export async function GET() {
  try {
    ...
    const activeCount = licenses.filter((l) => l.expirationDate > thirtyDaysFromNow).length;
    const expiringCount = licenses.filter((l) => l.expirationDate >= now && l.expirationDate <= thirtyDaysFromNow).length;
    ...
    const monthlySignups: { month: string; signups: number } = [];
    for (let i = 5; i >= 0; i--) {
