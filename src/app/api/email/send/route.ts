	...
	try {
		...
		if (!["owner", "admin"].includes(orgMember.role)) {
			return NextResponse.json(
				{
					error:
						"Insufficient permissions. Only owners and admins can send emails.",
				},
				{ status: 403 },
			);
		}
		...
		if (preferences?.alertEmailCategories === "critical_only") {
			const criticalTemplates: TemplateName[] = [
				"expiration_alert",
				"insurance_expiration",
				"password_reset",
			];
			if (!criticalTemplates.includes(template)) {
				return NextResponse.json(
					{
						error:
							"Your email preferences are set to critical-only. This template type is not critical.",
					},
					{ status: 400 },
				);
			}
