...
export async function GET() {
  try {
    ...
    const projects = await db.project.findMany({ where: { orgId, status: 'active' } });
    ...
    const licenseStates = new Set(licenses.filter((l) => l.expirationDate > now).map((l) => l.state).filter(Boolean));
    ...
    return NextResponse.json({
      ...
      coverage: {
        ...
        coveragePercent: Math.round((coveredStates.length / usStates.length) * 100),
      },
    });
  } catch (error) {
