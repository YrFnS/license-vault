...
export async function GET() {
	try {
		...
		locations.forEach((l) => {
			if (l.state) orgStates.add(l.state);
		});
		...
		const stateCompliance = Array.from(orgStates).map((state) => {
			...
			const activeCount = stateLicenses.filter(
				(l) => l.expirationDate > now,
			).length;
			const expiringCount = stateLicenses.filter(
				(l) => l.expirationDate > now && l.expirationDate <= thirtyDaysFromNow,
			).length;
			const expiredCount = stateLicenses.filter(
				(l) => l.expirationDate <= now,
			).length;
			...
			const missingLicenseTypes = requiredLicenseTypes.filter(
				(t) => !existingLicenseTypes.has(t),
			);
			...
			const ceHoursRequired = stateReqs.reduce(
				(sum, r) => sum + r.ceHoursRequired,
				0,
			);
			...
			const hasBond = insuranceBonds.some(
				(ib) => ib.type === "bond" && ib.status === "active",
			);
			const hasInsurance = insuranceBonds.some(
				(ib) => ib.type === "insurance" && ib.status === "active",
			);
				...
				licenses: stateLicenses.map((l) => ({
					...
					status:
						l.expirationDate <= now
							? "expired"
							: l.expirationDate <= thirtyDaysFromNow
								? "expiring_soon"
								: "active",
				})),
			};
		...
		const overallScore =
			totalLicenses > 0 ? Math.round((totalActive / totalLicenses) * 100) : 100;
