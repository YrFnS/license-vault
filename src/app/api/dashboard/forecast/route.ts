...
export async function GET(request: Request) {
	try {
		...
		const licenseItems = licenses.map((license) => {
			...
			const ceHoursEarned = license.ceTrackings.reduce(
				(sum, ce) => sum + ce.hoursEarned,
				0,
			);
			const ceHoursRequired = matchingReq?.ceHoursRequired || 0;
			const renewalFee = matchingReq
				? (matchingReq.renewalFeeMin + matchingReq.renewalFeeMax) / 2
				: 0;
		...
		const insuranceItems = insuranceBonds.map((bond) => {
			const daysRemaining = Math.ceil(
				(new Date(bond.expirationDate).getTime() - now.getTime()) /
					(1000 * 60 * 60 * 24),
			);
			...
			return {
				...
				daysRemaining,
				status:
					bond.expirationDate < now
						? "expired"
						: bond.expirationDate <= thirtyDaysFromNow
							? "expiring_soon"
							: "active",
				riskScore: calculateRiskScore(daysRemaining),
		...
		const orgRiskScore =
			allItems.length > 0
				? Math.round(
						allItems.reduce((sum, item) => sum + item.riskScore, 0) /
							allItems.length,
					)
				: 0;
		...
		if (scenario === "skip_renewal" && skipLicenseId) {
			...
			if (skippedItem) {
				...
				const activeRemaining = remainingItems.filter(
					(i) => i.status === "active",
				).length;
				...
				const financialExposure =
					skippedItem.renewalFee +
					atRiskItems.reduce((sum, i) => sum + i.renewalFee, 0);
				...
				whatIfResult = {
					...
					financialExposure,
					impact:
						newComplianceScore < 50
							? "critical"
							: newComplianceScore < 75
								? "high"
...
function buildForecast(
	...
	const newItemsNeeded = expiringItems.filter(
		(i) => !i.status.includes("active"),
	).length;
	const estimatedCost = expiringItems.reduce((sum, i) => sum + i.renewalFee, 0);
	const ceHoursNeeded = expiringItems.reduce(
		(sum, i) => sum + i.ceHoursNeeded,
		0,
	);
