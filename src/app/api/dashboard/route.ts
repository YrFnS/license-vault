...
export async function GET(request: Request) {
	try {
		...
		const monthlyActivity: Array<{ month: string; created: number }> = [];
		for (let i = 5; i >= 0; i--) {
			...
			const monthLabel = monthDate.toLocaleString(userLocale, {
				month: "short",
			});
		...
		const expiringLicenses = licenses
			.filter(
				(l) => l.expirationDate <= thirtyDaysFromNow || l.expirationDate < now,
			)
			.sort(
				(a, b) =>
