...
export function StatusDot({ status }: { status: string }) {
	const color =
		status === "compliant" || status === "active"
			? "bg-emerald-500"
...
export function ComplianceBadge({ className }: { className?: string }) {
	return (
		<Badge
			variant="outline"
			className={cn(
				"text-xs font-medium bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800",
				className,
			)}
		>
			Compliant
