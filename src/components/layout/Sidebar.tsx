				...
				<nav className="flex flex-col gap-4 px-2 py-3">
					{navSections.map((section) => {
						const items = section.items.filter((item) => {
							if (
								"requireAdmin" in item &&
								item.requireAdmin &&
								!canAccessAdmin
							)
								return false;
							if ("requireManage" in item && item.requireManage && !canManage)
								return false;
							return true;
						});
...
export function Sidebar({ className }: SidebarProps) {
	return (
		<aside
			className={cn(
				"hidden lg:flex flex-col border-r border-slate-200 dark:border-slate-800 h-screen w-[220px] shrink-0",
				className,
			)}
		>
			<SidebarContent />
