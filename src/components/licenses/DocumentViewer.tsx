...
function getCategoryColor(category: string): string {
  const colorMap: Record<string, string> = {
    license_copy: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400',
    ...
    ce_certificate: 'bg-cyan-100 text-cyan-800 dark:bg-cyan-900/30 dark:text-cyan-400',
...
function getFileTypeIcon(fileType: string) {
  switch (fileType.toLowerCase()) {
    ...
    case 'jpeg':
    case 'png':
      return <ImageIcon className="size-6 text-emerald-500 dark:text-emerald-400" />;
    default:
          ...
          {viewable ? (
            <div className="relative w-full" style={{ minHeight: '500px' }}>
              {loading && (
                <div className="absolute inset-0 flex items-center justify-center bg-muted/30">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-sm text-muted-foreground">{t('viewDocument')}</p>
                  </div>
                </div>
              )}
              {error && (
              ...
              {document.fileType.toLowerCase() === 'pdf' ? (
                ...
                />
              ) : ['jpg', 'jpeg', 'png'].includes(document.fileType.toLowerCase()) ? (
                <div className="flex items-center justify-center p-6 bg-muted/20 min-h-[500px]">
                  <img
                    ...
                    onError={() => {
                      setLoading(false);
                      setError('Unable to display image. Try downloading instead.');
                    }}
                  />
                </div>
              ) : null}
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1 max-w-xs">
                This file type ({document.fileType.toUpperCase()}) cannot be previewed in the browser.
              </p>
              <Button
