...
const AI_DOMAIN = process.env.AI_API_URL
...
export default function middleware(request: NextRequest) {
	...
	if (pathname.startsWith("/api/")) {
		...
		if (pathname.startsWith("/api/auth/")) {
			...
			limitResult = checkRateLimit(
				`public:${ip}`,
				PUBLIC_API_LIMIT.max,
				PUBLIC_API_LIMIT.window,
			);
		} else {
			limitResult = checkRateLimit(
				`api:${ip}`,
				GENERAL_API_LIMIT.max,
				GENERAL_API_LIMIT.window,
			);
		}
		...
		const isSendScheduledRoute = pathname.startsWith(
			"/api/reports/send-scheduled",
		);
		...
		if (
			...
			const allowedOrigins = [
				host || '',
				`localhost:${request.nextUrl.port ?? devPort}`,
			];
		...
		response.headers.set(
			"X-RateLimit-Reset",
			String(Date.now() + limitResult.resetIn),
		);
