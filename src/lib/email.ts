...
const SMTP_FROM =
	...
	`License Vault <${process.env.SMTP_FROM_EMAIL || "noreply@localhost"}>`; // Configure SMTP_FROM env var for production email
...
export async function sendExpirationAlert(
	...
	const daysLabel =
		data.daysUntil <= 0
			? "EXPIRED"
			: `${data.daysUntil} day${data.daysUntil !== 1 ? "s" : ""}`;
	...
	return sendEmail(
		...
		},
		{
			...
			context: {
				...
				daysUntil: String(data.daysUntil),
			},
		},
	);
}
...
export async function sendTemplateEmail(
	...
	switch (template) {
		case "expiration_alert":
			return sendExpirationAlert(
				to,
				data as unknown as Omit<ExpirationAlertData, "appUrl">,
				orgId,
			);
		case "insurance_expiration":
			return sendInsuranceExpirationAlert(
				to,
				data as unknown as Omit<InsuranceExpirationAlertData, "appUrl">,
				orgId,
			);
		case "password_reset":
			return sendPasswordReset(to, data as unknown as PasswordResetData, orgId);
		case "team_invitation":
			return sendTeamInvitation(to, data as unknown as TeamInvitationData, orgId);
		case "renewal_confirmation":
			return sendRenewalConfirmation(
				to,
				data as unknown as Omit<RenewalConfirmationData, "appUrl">,
				orgId,
			);
		case "renewal_reminder":
			return sendRenewalReminder(
				to,
				data as unknown as Omit<RenewalReminderData, "appUrl">,
				orgId,
			);
		case "compliance_report":
			return sendComplianceReport(to, data as unknown as ComplianceReportData, orgId);
		case "subcontractor_portal_invite":
			return sendSubcontractorPortalInvite(
				to,
				data as unknown as SubcontractorPortalInviteData,
				orgId,
			);
		case "welcome":
			return sendWelcomeEmail(to, data as unknown as WelcomeEmailData, orgId);
		case "test":
			return sendTestEmail(to, data as unknown as TestEmailData, orgId);
		default:
